...
//globo
float Menu::Settings::GlareX = 0.5050f;
float Menu::Settings::GlareY = 0.4954f;
float Menu::Settings::Glarewidth = 1.0240f;
float Menu::Settings::Glareheight = 1.0215f;

float Menu::Settings::menuX = 0.17f;
float Menu::Settings::Scroll = 0.0010f;
...
float Menu::Settings::menuY = 0.089f;
float Menu::Settings::menuH = 0.107f;
void Menu::Title(const char* title)
{
	//0.164f
	Drawing::Spriter("CommonMenu", "interaction_bgd", Settings::menuX, Settings::menuY, 0.21f, Settings::menuH, 0, Settings::titleRect.r, Settings::titleRect.g, Settings::titleRect.b, Settings::titleRect.a);
	Globe::drawGlare(Menu::Settings::GlareX, Menu::Settings::GlareY, Menu::Settings::Glarewidth, Menu::Settings::Glareheight, 0, 255, 255);
...
bool Menu::WeaponOption(char* weapon_Name, char* hash, char* hashment, int ammo, int tnit, bool attchment, bool tint, bool gun)
{
	Option(weapon_Name);
	if (Settings::currentOption == Settings::optionCount)
	{
		...
		if (Settings::selectPressed)
		{
			...
			if (attchment)WEAPON::GIVE_WEAPON_COMPONENT_TO_PED(PLAYER::PLAYER_PED_ID(), WeapHash, GAMEPLAY::GET_HASH_KEY(hashment));
			if (tint)WEAPON::SET_PED_WEAPON_TINT_INDEX(PLAYER::PLAYER_PED_ID(), WeapHash, tnit);
			if (gun) WEAPON::GIVE_WEAPON_TO_PED(PLAYER::PLAYER_PED_ID(), GAMEPLAY::GET_HASH_KEY(hash), ammo, false, true);
...
bool Menu::Floathundr(const char* option, float& _float, int min, int max)
{
	Option(option);

	if (Settings::optionCount == Settings::currentOption) {
		if (Settings::leftPressed) {
			_float <= min ? _float = max : _float /= 10;
		}
		if (Settings::rightPressed) {
			_float >= max ? _float = min : _float *= 10;
		}
...
bool Menu::Axis(const char* option, float& _float, int min, int max)
{
	Option(option);

	if (Settings::optionCount == Settings::currentOption) {
		if (Settings::leftPressed) {
			_float <= min ? _float = max : _float -= Menu::Settings::Scroll;
		}
		if (Settings::rightPressed) {
			_float >= max ? _float = min : _float += Menu::Settings::Scroll;
		}
		_float < min ? _float = max : _float > max ? _float = min : NULL;
	}
	...
	if (Settings::optionCount == Settings::currentOption && Settings::selectPressed) return true;
	if (Settings::optionCount == Settings::currentOption && Settings::leftPressed) return true;
	else if (Settings::optionCount == Settings::currentOption && Settings::rightPressed) return true;
	return false;
}
void Menu::End()
...
bool Menu::OptionProp(char* Prop_Name)
{
	...
	Hash hash = GAMEPLAY::GET_HASH_KEY(Prop_Name);
	...
	if (Settings::currentOption == Settings::optionCount)
	{
		...
		if (Settings::selectPressed)
		{
			Sponner::ObjectString = Prop_Name;
...
bool Menu::ColourOption(char* colour_Name, int colour_index, int colour_index2, bool Pearlescent, bool WheelColor)
{
	...
	if (Settings::currentOption == Settings::optionCount)
	{
		...
		if (Settings::selectPressed)
		{
			...
			if (Pearlescent)
			{
				...
				VEHICLE::SET_VEHICLE_EXTRA_COLOURS(veh, colour_index, wheelColor);

			}
			else if (WheelColor)
			{
				VEHICLE::GET_VEHICLE_EXTRA_COLOURS(veh, &pearlescentColor, &wheelColor);
				VEHICLE::SET_VEHICLE_EXTRA_COLOURS(veh, pearlescentColor, colour_index);
			}
			else
			{
				VEHICLE::SET_VEHICLE_MOD_KIT(veh, colour_index);
				VEHICLE::SET_VEHICLE_COLOURS(veh, colour_index, colour_index2);
			}
			return true;
		}
	}
...
bool Optiontext(const char* option, char* maptext)
{
	Option(option);
	...
	if (Menu::Settings::currentOption == Menu::Settings::optionCount)
	{
		...
		{
			if (Menu::Settings::currentOption)
				notifyBottom(maptext);
...
bool tick_ = false;
bool Menu::tick(const char* option)
{
	Settings::optionCount++;
	bool onThis = Settings::currentOption == Settings::optionCount ? true : false;
	if (Settings::currentOption <= 16 && Settings::optionCount <= 16)
	{
		if (tick_)
			Drawing::Spriter("commonmenu", "common_medal", Settings::menuX + 0.078f, (Settings::optionCount * 0.035f + 0.141f), 0.02f, 0.03f, 0, 0, 255, 0, 255);
...
bool Menu::Toggle(const char* option, bool& b00l)
{
	...
	if (b00l)
	{
		if (Settings::currentOption <= Settings::maxVisOptions && Settings::optionCount <= Settings::maxVisOptions)
			...
			Drawing::Spriter("commonmenu", "common_medal", Settings::menuX + 0.078f, ((Settings::optionCount - (Settings::currentOption - 16)) * 0.035f + 0.141f), 0.02f, 0.03f, 0, 0, 255, 0, 255);
	}
	else
	{
		if (Settings::currentOption <= Settings::maxVisOptions && Settings::optionCount <= Settings::maxVisOptions)
			Drawing::Spriter("commonmenu", "common_medal", Settings::menuX + 0.078f, (Settings::optionCount * 0.035f + 0.141f), 0.02f, 0.03f, 0, 255, 0, 70, 255);
		else if (Settings::optionCount > Settings::currentOption - Settings::maxVisOptions && Settings::optionCount <= Settings::currentOption)
			Drawing::Spriter("commonmenu", "common_medal", Settings::menuX + 0.078f, ((Settings::optionCount - (Settings::currentOption - 16)) * 0.035f + 0.141f), 0.02f, 0.03f, 0, 255, 0, 70, 255);
	}
	if (Settings::optionCount == Settings::currentOption && Settings::selectPressed) {
		b00l ^= 1;
		return true;
...
bool Menu::Float(const char* option, float& _float, int min, int max, int step)
{
	...
	if (Settings::optionCount == Settings::currentOption) {
		if (Settings::leftPressed) {
			_float <= min ? _float = max : _float -= 15.0f;
		}
		if (Settings::rightPressed) {
			_float >= max ? _float = min : _float += 15.0f;
		}
		_float < min ? _float = max : _float > max ? _float = min : NULL;
	}
...
int Menu::Settings::openKey = VK_INSERT;
	...
	if (GetTickCount() - Settings::keyPressPreviousTick > Settings::keyPressDelay) {
		if (GetTickCount() - Settings::keyPressPreviousTick2 > Settings::keyPressDelay2) {
			if (GetTickCount() - Settings::keyPressPreviousTick3 > Settings::keyPressDelay3) {
				if (IsKeyPressed(VK_INSERT) || CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlScriptRB) && CONTROLS::IS_DISABLED_CONTROL_PRESSED(0, ControlPhoneSelect) && Settings::controllerinput) {
...
static Entity Veh;
static int timer;
Hash hash = GAMEPLAY::GET_HASH_KEY(hashy);
Ped Player = PLAYER::PLAYER_PED_ID();
float heading = ENTITY::GET_ENTITY_HEADING(PLAYER::PLAYER_PED_ID());
if (Menu::Settings::currentOption == Menu::Settings::optionCount)
{
	if (STREAMING::IS_MODEL_VALID(hash))
	{
		...
		Vector3 Coords = ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(Player, 0, 4.0f, 0.5f);
		Veh = OBJECT::CREATE_OBJECT(hash, Coords.x, Coords.y, Coords.z, NULL, true, true);
		// VEHICLE::SET_VEHICLE_ON_GROUND_PROPERLY(Veh);
		ENTITY::SET_ENTITY_COLLISION(Veh, false, false);
		ENTITY::SET_ENTITY_ALPHA(Veh, 150, 0);
		STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(hash);
